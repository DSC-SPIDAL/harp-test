...
public class Driver {
  ...
  private static final String bcast_script = Depl
    .getProjectHomePath()
  ...
  private static final String graph_script = Depl
    .getProjectHomePath()
    + Depl.getBinDirectory()
    + "collective/graph.sh";
  private static final String event_script = Depl
    .getProjectHomePath()
    + Depl.getBinDirectory()
    + "collective/event.sh";
  private static final String lgs_script = Depl
    .getProjectHomePath()
    + Depl.getBinDirectory()
    + "collective/lgs.sh";
  ...
  public static void main(String args[])
    ...
    if (task.equals("bcast")) {
      ...
      // args[4]: numLoops
      isRunning =
        startAllWorkers(workers, bcast_script,
          driverHost, driverPort, jobID, args[3],
          args[4]);
    } else if (task.equals("regroup")) {
      // args[3]: partitionByteData
      // args[4]: numPartitions
      isRunning =
        startAllWorkers(workers, regroup_script,
          driverHost, driverPort, jobID, args[3],
          args[4]);
      ...
      // args[4]: numPartitions
      isRunning =
        startAllWorkers(workers, lgs_script,
          driverHost, driverPort, jobID, args[3],
          args[4]);
      ...
      // args[4]: numPartitions
      isRunning =
        startAllWorkers(workers, reduce_script,
          driverHost, driverPort, jobID, args[3],
          args[4]);
  ...
  public static boolean reportToDriver(
    ...
    transList.add(ack);
    Data ackData =
      new Data(DataType.SIMPLE_LIST, contextName,
        workerID, transList,
