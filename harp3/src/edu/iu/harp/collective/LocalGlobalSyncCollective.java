...
public class LocalGlobalSyncCollective {
  ...
  public static void main(String args[])
    ...
    try {
      ...
      push(contextName, "local-global",
        localTable, globalTable, new Partitioner(
          workers.getNumWorkers()), dataMap,
        workers);
    ...
    if (bcastPartitions.size() > 0) {
      Data data =
        new Data(DataType.PARTITION_LIST,
          contextName, selfID, bcastPartitions,
          size, operationName,
        ...
        for (Partition<P> partition : partitionList) {
          ...
          if (size + partitionSize > Constant.MAX_ARRAY_SIZE) {
            sendPartitions(contextName,
              operationName, selfID,
              sendOrder[i], sendPartitions,
              (int) size, workers);
  ...
  private static <P extends Simple> void copy(
    ...
    for (Partition<P> partition : ownedPartitions) {
      long partitionSize =
        partition.getNumEnocdeBytes();
      if (size + partitionSize > Constant.MAX_ARRAY_SIZE) {
        copyPartitions(contextName,
          operationName, selfID, localPartitions,
          (int) size, workers, dataMap);
  ...
  private static void copyPartitions(
    ...
    if (localPartitions.size() > 0) {
      Data data =
        new Data(DataType.PARTITION_LIST,
          contextName, selfID, localPartitions,
          size, operationName,
      ...
      if ((newData.getHeadStatus() == DataStatus.ENCODED_ARRAY_DECODED && newData
  ...
  public static
    ...
    List<Transferable> commList =
      new LinkedList<>();
    boolean isSuccess =
      PartitionUtil.allgatherPartitionSet(
        contextName, operationName
          + ".allgather.global", globalTable,
        commList, dataMap, workers);
    if (!isSuccess) {
      return false;
    }
    // ------------------------------------------
    ...
    isSuccess =
      PartitionUtil.regroupPartitionCount(
        contextName, operationName
          + ".regroup.local", localTable,
        commList, globalTableMap, partitioner,
        dataMap, workers);
    if (!isSuccess) {
      return false;
    }
  ...
  public static
    ...
    int numRecvPartitions =
      PartitionUtil.regroupPartitionSet(
        contextName, operationName
          + ".regroup.local", localTable,
        commList, globalParDistrMap, null,
        dataMap, workers);
  ...
  private static
    ...
    rotateGlobal(final String contextName,
      ...
      int destID, DataMap dataMap, Workers workers) {
    ...
    shift(contextName, operationName,
      globalTable, selfID, destID, workers);
    return PartitionUtil.receivePartitions(
      contextName, operationName, globalTable,
  ...
  private static <P extends Simple> void shift(
    ...
    for (Partition<P> partition : table
      ...
      if (size + partitionSize > Constant.MAX_ARRAY_SIZE) {
        sendPartitions(contextName,
          operationName, selfID, destID,
          sendPartitions, (int) size, workers);
        sendPartitions.clear();
        sendPartitions.add(partition);
        size = partitionSize;
        count++;
  ...
  private static void sendPartitions(
    ...
    if (sendPartitions.size() > 0) {
      Data data =
        new Data(DataType.PARTITION_LIST,
          contextName, selfID, sendPartitions,
          size, operationName,
          sendPartitions.size());
      DataSender sender =
