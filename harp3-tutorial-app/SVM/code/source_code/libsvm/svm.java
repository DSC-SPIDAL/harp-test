...
class Cache {
	...
	int get_data(int index, float[][] data, int len)
	{
		...
		if(more > 0)
		{
			...
			h.data = new_data;
			size -= more;
			do {int swap_tmp=h.len; h.len=len; len=swap_tmp;} while(false);
		}
	...
	void swap_index(int i, int j)
	{
		...
		do {float[] swap_tmp=head[i].data; head[i].data=head[j].data; head[j].data=swap_tmp;} while(false);
		do {int swap_tmp=head[i].len; head[i].len=head[j].len; head[j].len=swap_tmp;} while(false);
		if(head[i].len > 0) lru_insert(head[i]);
		if(head[j].len > 0) lru_insert(head[j]);

		if(i>j) do {int swap_tmp=i; i=j; j=swap_tmp;} while(false);
		for(head_t h = lru_head.next; h!=lru_head; h=h.next)
		{
			if(h.len > i)
			{
				if(h.len > j)
					do {float swap_tmp=h.data[i]; h.data[i]=h.data[j]; h.data[j]=swap_tmp;} while(false);
				else
				{
					// give up
					lru_delete(h);
...
abstract class Kernel extends QMatrix {
	...
	void swap_index(int i, int j)
	{
		do {svm_node[] swap_tmp=x[i]; x[i]=x[j]; x[j]=swap_tmp;} while(false);
		if(x_square != null) do {double swap_tmp=x_square[i]; x_square[i]=x_square[j]; x_square[j]=swap_tmp;} while(false);
	}
...
class Solver {
	...
	{
		Q.swap_index(i,j);
		do {byte swap_tmp=y[i]; y[i]=y[j]; y[j]=swap_tmp;} while(false);
		do {double swap_tmp=G[i]; G[i]=G[j]; G[j]=swap_tmp;} while(false);
		do {byte swap_tmp=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=swap_tmp;} while(false);
		do {double swap_tmp=alpha[i]; alpha[i]=alpha[j]; alpha[j]=swap_tmp;} while(false);
		do {double swap_tmp=p[i]; p[i]=p[j]; p[j]=swap_tmp;} while(false);
		do {int swap_tmp=active_set[i]; active_set[i]=active_set[j]; active_set[j]=swap_tmp;} while(false);
		do {double swap_tmp=G_bar[i]; G_bar[i]=G_bar[j]; G_bar[j]=swap_tmp;} while(false);
	}
...
class SVC_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		...
		do {byte swap_tmp=y[i]; y[i]=y[j]; y[j]=swap_tmp;} while(false);
		do {double swap_tmp=QD[i]; QD[i]=QD[j]; QD[j]=swap_tmp;} while(false);
	}
}
...
class SVR_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		do {byte swap_tmp=sign[i]; sign[i]=sign[j]; sign[j]=swap_tmp;} while(false);
		do {int swap_tmp=index[i]; index[i]=index[j]; index[j]=swap_tmp;} while(false);
		do {double swap_tmp=QD[i]; QD[i]=QD[j]; QD[j]=swap_tmp;} while(false);
	}
...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<prob.l;i++)
		{
			...
			do {int swap_tmp=perm[i]; perm[i]=perm[j]; perm[j]=swap_tmp;} while(false);
		}
		for(i=0;i<nr_fold;i++)
		{
	...
	private static void svm_group_classes(svm_problem prob, int[] nr_class_ret, int[][] label_ret, int[][] start_ret, int[][] count_ret, int[] perm)
	{
		...
		{
			do {int swap_tmp=label[0]; label[0]=label[1]; label[1]=swap_tmp;} while(false);
			do {int swap_tmp=count[0]; count[0]=count[1]; count[1]=swap_tmp;} while(false);
			for(i=0;i<l;i++)
			{
				if(data_label[i] == 0)
					data_label[i] = 1;
		    ...
		    param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
		{
			...
			for (c=0; c<nr_class; c++)
				for(i=0;i<count[c];i++)
				{
					...
					do {int swap_tmp=index[start[c]+j]; index[start[c]+j]=index[start[c]+i]; index[start[c]+i]=swap_tmp;} while(false);
				}
			for(i=0;i<nr_fold;i++)
			{
				fold_count[i] = 0;
