...
public class NeuralNetwork implements Serializable
{
	...
	public int [] itsTopology; 			//Neural network topology. each element inidcates neuron size on its layer.
	public Vector<DoubleMatrix> itsTheta;	//Weight matrices between each layer.
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			DoubleMatrix dataX = NeuralNetwork.getMatrixFromTextFile("digitInput.txt");	//Get training input data from a file
			DoubleMatrix dataY = NeuralNetwork.getMatrixFromTextFile("digitOutput.txt");	//Get training output data from a file

			// DoubleMatrix X = new DoubleMatrix(new double[][]{{1d, 2d, 3d}, {4d, 5d, 6d}, {7d, 8d, 9d}, {10d, 11d, 12d}});
			// DoubleMatrix Y = new DoubleMatrix(new double[][]{{1d, 0d}, {1d, 0d}, {1d, 1d}, {0d,0d}});

			int[] topology = {dataX.columns,10, dataY.columns};	
			double lambda = 0.6987;//0.905343;

			int inputVecSize = dataX.columns;
			int sizeOfLayer = 10;
			int outputVecSize = dataY.columns;
			...
			NeuralNetwork digitReader = new NeuralNetwork(topology, false);	//Create the neural network! randomly generate theta elements

			for(int s = 0; s < 10 ; s++)
			{
				random.setSeed(s);
				digitReader.weight_init(inputVecSize, sizeOfLayer, outputVecSize, s);
				...
				DoubleMatrix X = digitReader.getMiniBatch(dataX, shuffledList);
				DoubleMatrix Y = digitReader.getMiniBatch(dataY, shuffledList);
				...
				DoubleMatrix testX = digitReader.getMiniBatch(dataX, testSeq);
				DoubleMatrix testY = digitReader.getMiniBatch(dataY, testSeq);

				double accuracy = NeuralNetwork.computeAccuracy(digitReader.predictFP(testX), testY); //find the accuracy
	...
	public DoubleMatrix getMiniBatch(DoubleMatrix dataMat, List shuffledList) throws IOException
	{
	...
	public void weight_init(int inputVecSize, int sizeOfLayer,int outputVecSize, int seed) throws IOException
	{
		double w;
   ...
   //  		File file = new File(localDirStr + File.separator + filename);
   //  		FileWriter fw = new FileWriter(file.getAbsoluteFile());
			// BufferedWriter bw = new BufferedWriter(fw);
			
			for (int k = 0; k < weightVectorLength; k++) 
	    	{
				w = random.nextDouble();
				flatWeightMatrix.put(k, 0, w);
				// bw.write(w+" ");	
    		}
    		// bw.newLine();
	    	// bw.close();	
	    	// System.out.println("Done written weights to file "+ filename);
	    ...
	    // fs.copyFromLocalFile(localPath, wDir);
	}
	...
	public static Vector<DoubleMatrix> generateThetas(int[] topology)
	{
		...
		{
			ThetaMatrix.add(DoubleMatrix.rand(topology[i+1],topology[i]+1));
		}
		return ThetaMatrix;
	}
