...
import net.sf.javaml.tools.sampling.SubSamplingNew;
...
public class Bagging extends AbstractClassifier {
	...
	public Bagging(Classifier[] classifiers, Random rg) {
		...
		this.samplingMethod = new SubSamplingNew();
	}
	...
	*/
	...
	public void buildClassifierNew(Dataset data) {
		this.dataReference = data;
		int t = 0, f = 0;
		for (int i = 0; i < classifiers.length; i++) {
			System.out.println("Building tree: " + i);
			Dataset sampleTrain = DatasetTools.sampleTrain(data, samplingMethod, data
					.size(), seed++);
			classifiers[i].buildClassifier(sampleTrain);			
		}	
	}
